...
- **배포가 아직 성립하지 않았습니다.** 배포 블로커(corpus 번들 포함, Mock Bank 인메모리, 진입점)는 해소했지만, Vercel 라우팅이 맞지 않아 현재 배포본은 모든 경로가 404입니다. 프로젝트 framework 프리셋이 `python`이라 FastAPI 함수가 도메인 전체를 잡는데, `vercel.json` rewrite가 함수를 `/api/*` 아래로 가정하고 경로에 `/api`를 덧붙이고 있습니다.

...
- [ ] **corpus에 같은 문서의 사본이 102종 있다.** `예금거래기본약관`이 파일명만 다른 3벌,
      `약관 및 상품설명서`가 76청크 등. 검색이 동일 조항을 5건까지 그대로 돌려줘서 근거
      슬롯이 낭비된다(retrieval의 본문 정규화 dedup은 PDF 추출 차이 때문에 못 걸러냄).
      화면에서는 "사본 n건"으로 접어 가렸지만 근본 해결은 `data/products/`에서 중복 PDF를
      정리하고 corpus를 다시 만드는 것이다.

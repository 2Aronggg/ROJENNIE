...
- `server/mcp/finance/client.py`: Finance MCP Tool 호출
- `server/mcp/finance/finance_server.py`: 가상 금융정보를 읽기 전용 Tool로 제공
- `server/rag/retrieval.py`: `data/`의 로컬 문서 검색
- `server/agents/rag_query.py`: LLM 또는 규칙 기반 RAG 검색어 생성
- `server/agents/logic_verification.py`: 사실·계약·거래·근거의 연결 검증
